...
: 2018.02.22 WVG at BBP: Added TTX switch
...
BREAKPOINT {
    ...
    : Code below disables channel when TTX is enabled
    : This should be improved by affecting the real state vars
 	if (ttxi == 0.015625 && ttxo > 1e-12) {
        : Set number of open channels to 0 when TTX enabled
        ttx_O = 0
    } else {     
        : If no TTX use normal O value               
        ttx_O = O
    }       
	g = gbar * ttx_O
	ina = g * (v - ena)
